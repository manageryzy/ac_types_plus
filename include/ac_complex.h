...
public:
  ...
  inline ac_int<WS*2, false> slc(const ac_int<WX, SX> &index) const {
      ac_int<WS*2, false> r;
      r.set_slc(WS, _r.template slc<WS>(index));
      r.set_slc(0, _i.template slc<WS>(index));
      return r;
  }

  template<int WS>
  inline ac_int<WS*2, false> slc(signed index) const {
      ac_int<WS*2, false> r;
      r.set_slc(WS, _r.template slc<WS>(index));
      r.set_slc(0, _i.template slc<WS>(index));
      return r;
  ...
  template<int WS>
  inline ac_int<WS*2, false> slc(unsigned uindex) const {
      ac_int<WS*2, false> r;
      r.set_slc(WS, _r.template slc<WS>(uindex));
      r.set_slc(0, _i.template slc<WS>(uindex));
      return r;
  }

    //TODO:
  // template<int W2, bool S2, int WX, bool SX>
  // inline ac_complex<T> &set_slc(const ac_int<WX, SX> lsb, const ac_int<W2, S2> &slc) {
  //     AC_ASSERT(lsb.to_int() + W2 <= W && lsb.to_int() >= 0, "Out of bounds set_slc");
  //
  //     return *this;
  // }
  // template<int W2, bool S2>
  // inline ac_complex<T> &set_slc(signed lsb, const ac_int<W2, S2> &slc) {
  //     AC_ASSERT(lsb + W2 <= W && lsb >= 0, "Out of bounds set_slc");
  //
  //     return *this;
  // }
  // template<int W2, bool S2>
  // inline ac_complex<T> &set_slc(unsigned ulsb, const ac_int<W2, S2> &slc) {
