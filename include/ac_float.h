...
namespace __AC_NAMESPACE {
	...
	namespace ac_private {
		...
		CUDA_CALLABLE_MEMBER inline ac_float_cdouble_t double_to_ac_float(double d);
		CUDA_CALLABLE_MEMBER inline ac_float_cfloat_t float_to_ac_float(float f);
	}
	...
	public:
		...
		CUDA_CALLABLE_MEMBER void set_mantissa(const ac_fixed<W, I, S>& man) { m = man; }
		CUDA_CALLABLE_MEMBER void set_exp(const ac_int<E, true>& exp) { if (E) e = exp; }

	private:
		CUDA_CALLABLE_MEMBER inline bool is_neg() const { return m < 0; }   // is_neg would be more efficient
	...
	public:
		...
		CUDA_CALLABLE_MEMBER ac_float() {
#if defined(AC_DEFAULT_IN_RANGE)
#endif
		}
		CUDA_CALLABLE_MEMBER ac_float(const ac_float& op) {
			m = op.m;
			e = op.e;
		}
		template<ac_q_mode Q2>
		CUDA_CALLABLE_MEMBER ac_float(const ac_float<W, I, E, Q2>& op) {
			m = op.m;
			e = op.e;
		}
	...
	private:
		template<int W2>
		CUDA_CALLABLE_MEMBER bool round(const ac_fixed<W2, I, true>& op2, bool assert_on_rounding = false) {
		...
		CUDA_CALLABLE_MEMBER void assign_from(const ac_fixed<W2, I2, true, Q2, O2>& m2, int e2, bool sticky_bit, bool force_normalize, bool assert_on_overflow = false, bool assert_on_rounding = false) {
	...
	public:
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER ac_float(const AC_FL(2)& op, bool force_normalize = false, bool assert_on_overflow = false, bool assert_on_rounding = false) {
		...
		CUDA_CALLABLE_MEMBER ac_float(const ac_fixed<W, I, S>& m2, const ac_int<E, true>& e2, bool normalize = true) {
			m = m2;
			e = e2;
			if (normalize)
				this->normalize();
		...
		CUDA_CALLABLE_MEMBER ac_float(const ac_fixed<WFX, IFX, SFX, QFX, OFX>& m2, const ac_int<E2, true>& e2, bool normalize = true) {
		...
		CUDA_CALLABLE_MEMBER ac_float(const ac_fixed<WFX, IFX, SFX, QFX, OFX>& op) {
			assign_from<0, 0>(ac_fixed<WFX + !SFX, IFX + !SFX, true>(op), 0, false, true);
		}

		template<int WI, bool SI>
		CUDA_CALLABLE_MEMBER ac_float(const ac_int<WI, SI>& op) {
			*this = ac_fixed<WI, WI, SI>(op);
		}

		CUDA_CALLABLE_MEMBER inline ac_float(bool b) { *this = (ac_int<1, false>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(char b) { *this = (ac_int<8, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(signed char b) { *this = (ac_int<8, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(unsigned char b) { *this = (ac_int<8, false>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(signed short b) { *this = (ac_int<16, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(unsigned short b) { *this = (ac_int<16, false>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(signed int b) { *this = (ac_int<32, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(unsigned int b) { *this = (ac_int<32, false>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(signed long b) { *this = (ac_int<ac_private::long_w, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(unsigned long b) { *this = (ac_int<ac_private::long_w, false>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(Slong b) { *this = (ac_int<64, true>) b; }
		CUDA_CALLABLE_MEMBER inline ac_float(Ulong b) { *this = (ac_int<64, false>) b; }

		// Explicit conversion functions to ac_int and ac_fixed
		CUDA_CALLABLE_MEMBER inline typename rt_unary::to_ac_fixed_t to_ac_fixed() const {
			typename rt_unary::to_ac_fixed_t r = m;
			r <<= e;
			return r;
		}
		CUDA_CALLABLE_MEMBER inline typename rt_unary::to_ac_int_t to_ac_int() const {
			return to_ac_fixed().to_ac_int();
		}

		// Explicit conversion functions to C built-in types -------------
		CUDA_CALLABLE_MEMBER inline int to_int() const { return to_ac_int().to_int(); }
		CUDA_CALLABLE_MEMBER inline unsigned to_uint() const { return to_ac_int().to_uint(); }
		CUDA_CALLABLE_MEMBER inline long to_long() const { return (signed long)to_ac_int().to_int64(); }
		CUDA_CALLABLE_MEMBER inline unsigned long to_ulong() const { return (unsigned long)to_ac_int().to_uint64(); }
		CUDA_CALLABLE_MEMBER inline Slong to_int64() const { return to_ac_int().to_int64(); }
		CUDA_CALLABLE_MEMBER 		inline Ulong to_uint64() const { return to_ac_int().to_uint64(); }
		CUDA_CALLABLE_MEMBER inline float to_float() const { return ldexpf(m.to_double(), exp()); }
		CUDA_CALLABLE_MEMBER inline double to_double() const { return ldexp(m.to_double(), exp()); }

		CUDA_CALLABLE_MEMBER const ac_fixed<W, I, S> mantissa() const { return m; }
		CUDA_CALLABLE_MEMBER const ac_int<E, true> exp() const { return e; }
		CUDA_CALLABLE_MEMBER bool normalize() {
			bool all_sign;
		...
		CUDA_CALLABLE_MEMBER ac_float(double d, bool assert_on_overflow = false, bool assert_on_rounding = false) {
		...
		CUDA_CALLABLE_MEMBER ac_float(float f, bool assert_on_overflow = false, bool assert_on_rounding = false) {
		...
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER bool compare(const AC_FL(2)& op2, bool* gt) const {
		...
		CUDA_CALLABLE_MEMBER void plus_minus(const AC_FL(2)& op2, AC_FL(R)& r, bool sub = false) const {
		...
		CUDA_CALLABLE_MEMBER ac_float add(const AC_FL(1)& op1, const AC_FL(2)& op2) {
			op1.plus_minus(op2, *this);
			return *this;
		}

		template<AC_FL_T(1), AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER ac_float sub(const AC_FL(1)& op1, const AC_FL(2)& op2) {
			op1.plus_minus(op2, *this, true);
			return *this;
		}

		CUDA_CALLABLE_MEMBER typename rt_unary::neg abs() const {
		...
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER typename rt< AC_FL_TV0(2) >::plus operator +(const AC_FL(2)& op2) const {
			typename rt< AC_FL_TV0(2) >::plus r;
			plus_minus(op2, r);
			return r;
		}
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER typename rt< AC_FL_TV0(2) >::minus operator -(const AC_FL(2)& op2) const {
		...
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER typename rt< AC_FL_TV0(2) >::mult operator *(const AC_FL(2)& op2) const {
		...
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER typename rt< AC_FL_TV0(2) >::div operator /(const AC_FL(2)& op2) const {
			typename rt< AC_FL_TV0(2) >::div r(m / op2.m, exp() - op2.exp());
			return r;
		}
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER ac_float& operator +=(const AC_FL(2)& op2) {
			ac_float r;
			plus_minus(op2, r);
			*this = r;
			return *this;
		...
		CUDA_CALLABLE_MEMBER ac_float operator + () const {
			return *this;
		}
		...
		// Shift --------------------------------------------------------------------
		template<int WI, bool SI>
		CUDA_CALLABLE_MEMBER typename rt_i<WI, SI>::lshift operator << (const ac_int<WI, SI>& op2) const {
		...
		template<int WI, bool SI>
		CUDA_CALLABLE_MEMBER typename rt_i<WI, SI>::rshift operator >> (const ac_int<WI, SI>& op2) const {
		...
		// Shift assign -------------------------------------------------------------
		template<int WI, bool SI>
		CUDA_CALLABLE_MEMBER ac_float& operator <<= (const ac_int<WI, SI>& op2) {
			*this = operator << (op2);
			return *this;
		}
		template<int WI, bool SI>
		...
		template<AC_FL_T(2)>
		CUDA_CALLABLE_MEMBER bool operator == (const AC_FL(2)& f) const {
			bool gt;
			return compare(f, &gt);
		}
		template<AC_FL_T(2)>
		...
		CUDA_CALLABLE_MEMBER inline ac_float& set_val() {
	...
	namespace ac_private {
		template<typename T>
		CUDA_CALLABLE_MEMBER bool ac_fpclassify(T x, bool& inf) {
			bool nan = !(x == x);
			if (!nan) {
				T d = x - x;
				inf = !(d == d);
...
#define FL_BIN_OP_WITH_CTYPE(BIN_OP, C_TYPE, RTYPE)  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T2<C_TYPE>::RTYPE operator BIN_OP ( C_TYPE c_op, const AC_FL() &op) {  \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t; \
    return fl2_t(c_op).operator BIN_OP (op);  \
  } \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T<C_TYPE>::RTYPE operator BIN_OP ( const AC_FL() &op, C_TYPE c_op) {  \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t; \
...
#define FL_REL_OP_WITH_CTYPE(REL_OP, C_TYPE)  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( const AC_FL() &op, C_TYPE op2) {  \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t; \
    return op.operator REL_OP (fl2_t(op2));  \
  }  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( C_TYPE op2, const AC_FL() &op) {  \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t; \
...
#define FL_ASSIGN_OP_WITH_CTYPE_2(ASSIGN_OP, C_TYPE)  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline AC_FL() &operator ASSIGN_OP ( AC_FL() &op, C_TYPE op2) {  \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t; \
...
#define FL_SHIFT_OP_WITH_INT_CTYPE(BIN_OP, C_TYPE, RTYPE)  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_i< ac_private::c_type_params<C_TYPE>::W, ac_private::c_type_params<C_TYPE>::S >::RTYPE operator BIN_OP ( const AC_FL() &op, C_TYPE i_op) {  \
...
#define FL_SHIFT_ASSIGN_OP_WITH_INT_CTYPE(ASSIGN_OP, C_TYPE)  \
  template< AC_FL_T() > \
  CUDA_CALLABLE_MEMBER inline AC_FL() &operator ASSIGN_OP ( AC_FL() &op, C_TYPE i_op) {  \
    typedef typename ac::template ac_int_represent<C_TYPE>::type i_t; \
...
#define FL_BIN_OP_WITH_AC_INT_1(BIN_OP, RTYPE)  \
  ...
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T2< ac_int<WI,SI> >::RTYPE operator BIN_OP ( const ac_int<WI,SI> &i_op, const AC_FL() &op) {  \
...
#define FL_BIN_OP_WITH_AC_INT_2(BIN_OP, RTYPE)  \
  template< AC_FL_T(), int WI, bool SI> \
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T2< ac_int<WI,SI> >::RTYPE operator BIN_OP ( const AC_FL() &op, const ac_int<WI,SI> &i_op) {  \
    typedef typename ac::template ac_float_represent< ac_int<WI,SI> >::type fl2_t; \
...
#define FL_REL_OP_WITH_AC_INT(REL_OP)  \
  template< AC_FL_T(), int WI, bool SI> \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( const AC_FL() &op, const ac_int<WI,SI> &op2) {  \
    typedef typename ac::template ac_float_represent< ac_int<WI,SI> >::type fl2_t; \
    return op.operator REL_OP (fl2_t(op2));  \
  }  \
  template< AC_FL_T(), int WI, bool SI> \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( ac_int<WI,SI> &op2, const AC_FL() &op) {  \
    typedef typename ac::template ac_float_represent< ac_int<WI,SI> >::type fl2_t; \
    return fl2_t(op2).operator REL_OP (op);  \
  }
...
#define FL_ASSIGN_OP_WITH_AC_INT(ASSIGN_OP)  \
  template< AC_FL_T(), int WI, bool SI> \
  CUDA_CALLABLE_MEMBER inline AC_FL() &operator ASSIGN_OP ( AC_FL() &op, const ac_int<WI,SI> &op2) {  \
    typedef typename ac::template ac_float_represent< ac_int<WI,SI> >::type fl2_t; \
    return op.operator ASSIGN_OP (fl2_t(op2));  \
  }
...
#define FL_BIN_OP_WITH_AC_FIXED_1(BIN_OP, RTYPE)  \
  ...
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T2< ac_fixed<WF,IF,SF> >::RTYPE operator BIN_OP ( const ac_fixed<WF,IF,SF,QF,OF> &f_op, const AC_FL() &op) {  \
...
#define FL_BIN_OP_WITH_AC_FIXED_2(BIN_OP, RTYPE)  \
  template< AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF> \
  CUDA_CALLABLE_MEMBER inline typename AC_FL()::template rt_T2< ac_fixed<WF,IF,SF> >::RTYPE operator BIN_OP ( const AC_FL() &op, const ac_fixed<WF,IF,SF,QF,OF> &f_op) {  \
    typedef typename ac::template ac_float_represent< ac_fixed<WF,IF,SF> >::type fl2_t; \
...
#define FL_REL_OP_WITH_AC_FIXED(REL_OP)  \
  template< AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF> \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( const AC_FL() &op, const ac_fixed<WF,IF,SF,QF,OF> &op2) {  \
    typedef typename ac::template ac_float_represent< ac_fixed<WF,IF,SF> >::type fl2_t; \
    return op.operator REL_OP (fl2_t(op2));  \
  }  \
  template< AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF> \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( ac_fixed<WF,IF,SF,QF,OF> &op2, const AC_FL() &op) {  \
    typedef typename ac::template ac_float_represent< ac_fixed<WF,IF,SF> >::type fl2_t; \
    return fl2_t(op2).operator REL_OP (op);  \
  }
...
#define FL_ASSIGN_OP_WITH_AC_FIXED(ASSIGN_OP)  \
  template< AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF> \
  CUDA_CALLABLE_MEMBER inline AC_FL() &operator ASSIGN_OP ( AC_FL() &op, const ac_fixed<WF,IF,SF,QF,OF> &op2) {  \
    typedef typename ac::template ac_float_represent< ac_fixed<WF,IF,SF> >::type fl2_t; \
    return op.operator ASSIGN_OP (fl2_t(op2));  \
  }
...
// -------------------------------------------- End of Macros for Binary Operators with ac_fixed
	...
	CUDA_CALLABLE_MEMBER inline AC_FL() value(AC_FL()) {
		AC_FL() r;
	...
	namespace ac {
		...
		template<ac_special_val V, AC_FL_T() >
		CUDA_CALLABLE_MEMBER inline bool init_array(AC_FL()* a, int n) {
			AC_FL0() t;
