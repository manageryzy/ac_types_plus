...
#define AC_ASSERT(cond, msg) ac_private::ac_assert(cond, __FILE__, __LINE__, msg)
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline double ldexpr32(double d) {
      double d2 = d;
      if (N < 0)
        for (int i = 0; i < -N; i++)
          d2 /= (Ulong)1 << 32;
    ...
    template<> CUDA_CALLABLE_MEMBER inline double ldexpr32<0>(double d) { return d; }
    template<> CUDA_CALLABLE_MEMBER inline double ldexpr32<1>(double d) { return d * ((Ulong)1 << 32); }
    template<> CUDA_CALLABLE_MEMBER inline double ldexpr32<-1>(double d) { return d / ((Ulong)1 << 32); }
    template<> CUDA_CALLABLE_MEMBER inline double ldexpr32<2>(double d) { return (d * ((Ulong)1 << 32)) * ((Ulong)1 << 32); }
    template<> CUDA_CALLABLE_MEMBER inline double ldexpr32<-2>(double d) { return (d / ((Ulong)1 << 32)) / ((Ulong)1 << 32); }

    template<int N>
    CUDA_CALLABLE_MEMBER inline double ldexpr(double d) {
      return ldexpr32<N / 32>(N < 0 ? d / ((unsigned)1 << (-N & 31)) : d * ((unsigned)1 << (N & 31)));
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_copy(const int* op, int* r) {
      for (int i = 0; i < N; i++)
        r[i] = op[i];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_copy<1>(const int* op, int* r) {
      r[0] = op[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_copy<2>(const int* op, int* r) {
      r[0] = op[0];
      r[1] = op[1];
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_zero(const int* op) {
      for (int i = 0; i < N; i++)
        if (op[i])
          return false;
      return true;
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_zero<0>(const int* /*op*/) { return true; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_zero<1>(const int* op) {
      return !op[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_zero<2>(const int* op) {
      return !(op[0] || op[1]);
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_ones(const int* op) {
      for (int i = 0; i < N; i++)
        if (~op[i])
          return false;
      return true;
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_ones<0>(const int* /*op*/) { return true; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_ones<1>(const int* op) {
      return !~op[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal_ones<2>(const int* op) {
      return !(~op[0] || ~op[1]);
    }

    template<int N1, int N2>
    CUDA_CALLABLE_MEMBER inline bool iv_equal(const int* op1, const int* op2) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_equal<1, 1>(const int* op1, const int* op2) {
      return op1[0] == op2[0];
    }

    template<int B, int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_ones_from(const int* op) {
      if ((B >= 32 * N && op[N - 1] >= 0) || (B & 31 && ~(op[B / 32] >> (B & 31))))
        return false;
      return iv_equal_ones<N - (B + 31) / 32>(&op[(B + 31) / 32]);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_ones_from<0, 1>(const int* op) {
      return iv_equal_ones<1>(op);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_ones_from<0, 2>(const int* op) {
      return iv_equal_ones<2>(op);
    }

    template<int B, int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_zeros_from(const int* op) {
      if ((B >= 32 * N && op[N - 1] < 0) || (B & 31 && (op[B / 32] >> (B & 31))))
        return false;
      return iv_equal_zero<N - (B + 31) / 32>(&op[(B + 31) / 32]);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_zeros_from<0, 1>(const int* op) {
      return iv_equal_zero<1>(op);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_zeros_from<0, 2>(const int* op) {
      return iv_equal_zero<2>(op);
    }

    template<int B, int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_ones_to(const int* op) {
      if ((B >= 32 * N && op[N - 1] >= 0) || (B & 31 && ~(op[B / 32] | (all_ones << (B & 31)))))
        return false;
      return iv_equal_ones<B / 32>(op);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_ones_to<0, 1>(const int* op) {
      return iv_equal_ones<1>(op);
    }
    template<> CUDA_CALLABLE_MEMBER inline bool  iv_equal_ones_to<0, 2>(const int* op) {
      return iv_equal_ones<2>(op);
    }

    template<int B, int N>
    CUDA_CALLABLE_MEMBER inline bool iv_equal_zeros_to(const int* op) {
      if ((B >= 32 * N && op[N - 1] < 0) || (B & 31 && (op[B / 32] & ~(all_ones << (B & 31)))))
        return false;
      return iv_equal_zero<B / 32>(op);
    }
    template<> inline CUDA_CALLABLE_MEMBER bool  iv_equal_zeros_to<0, 1>(const int* op) {
      return iv_equal_zero<1>(op);
    }
    template<> inline CUDA_CALLABLE_MEMBER bool  iv_equal_zeros_to<0, 2>(const int* op) {
      return iv_equal_zero<2>(op);
    }

    template<int N1, int N2, bool greater>
    CUDA_CALLABLE_MEMBER inline bool iv_compare(const int* op1, const int* op2) {
      const int M1 = AC_MAX(N1, N2);
      const int M2 = AC_MIN(N1, N2);
      const int* OP1 = N1 >= N2 ? op1 : op2;
      const int* OP2 = N1 >= N2 ? op2 : op1;
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_extend(int* r, int ext) {
      for (int i = 0; i < N; i++)
        r[i] = ext;
    }
    template<> inline CUDA_CALLABLE_MEMBER void iv_extend<-2>(int* /*r*/, int /*ext*/) { }
    template<> inline CUDA_CALLABLE_MEMBER void iv_extend<-1>(int* /*r*/, int /*ext*/) { }
    template<> inline CUDA_CALLABLE_MEMBER void iv_extend<0>(int* /*r*/, int /*ext*/) { }
    template<> inline CUDA_CALLABLE_MEMBER void iv_extend<1>(int* r, int ext) {
      r[0] = ext;
    }
    template<> inline CUDA_CALLABLE_MEMBER void iv_extend<2>(int* r, int ext) {
    ...
    template<int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_assign_int64(int* r, Slong l) {
      r[0] = (int)l;
      if (Nr > 1) {
        r[1] = (int)(l >> 32);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_assign_int64<1>(int* r, Slong l) {
      r[0] = (int)l;
    }
    template<> CUDA_CALLABLE_MEMBER  inline void iv_assign_int64<2>(int* r, Slong l) {
      r[0] = (int)l;
      r[1] = (int)(l >> 32);
    }

    template<int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_assign_uint64(int* r, Ulong l) {
      r[0] = (int)l;
      if (Nr > 1) {
        r[1] = (int)(l >> 32);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_assign_uint64<1>(int* r, Ulong l) {
      r[0] = (int)l;
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_assign_uint64<2>(int* r, Ulong l) {
      r[0] = (int)l;
      r[1] = (int)(l >> 32);
    }

    CUDA_CALLABLE_MEMBER inline Ulong mult_u_u(int a, int b) {
      return (Ulong)(unsigned)a * (Ulong)(unsigned)b;
    }
    CUDA_CALLABLE_MEMBER inline Slong mult_u_s(int a, int b) {
      return (Ulong)(unsigned)a * (Slong)(signed)b;
    }
    CUDA_CALLABLE_MEMBER inline Slong mult_s_u(int a, int b) {
      return (Slong)(signed)a * (Ulong)(unsigned)b;
    }
    CUDA_CALLABLE_MEMBER inline Slong mult_s_s(int a, int b) {
      return (Slong)(signed)a * (Slong)(signed)b;
    }
    CUDA_CALLABLE_MEMBER inline void accumulate(Ulong a, Ulong& l1, Slong& l2) {
      l1 += (Ulong)(unsigned)a;
      l2 += a >> 32;
    }
    CUDA_CALLABLE_MEMBER inline void accumulate(Slong a, Ulong& l1, Slong& l2) {
      l1 += (Ulong)(unsigned)a;
      l2 += a >> 32;
    }

    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_mult(const int* op1, const int* op2, int* r) {
      if (Nr == 1)
        r[0] = op1[0] * op2[0];
      else if (N1 == 1 && N2 == 1)
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_mult<1, 1, 1>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] * op2[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_mult<1, 1, 2>(const int* op1, const int* op2, int* r) {
      iv_assign_int64<2>(r, ((Slong)op1[0]) * ((Slong)op2[0]));
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_uadd_carry(const int* op1, bool carry, int* r) {
      Slong l = carry;
      for (int i = 0; i < N; i++) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_uadd_carry<0>(const int* /*op1*/, bool carry, int* /*r*/) { return carry; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_uadd_carry<1>(const int* op1, bool carry, int* r) {
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_add_int_carry(const int* op1, int op2, bool carry, int* r) {
      if (N == 0)
        return carry;
      if (N == 1) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_add_int_carry<0>(const int* /*op1*/, int /*op2*/, bool carry, int* /*r*/) { return carry; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_add_int_carry<1>(const int* op1, int op2, bool carry, int* r) {
      Ulong l = carry + (Slong)op1[0] + (Slong)op2;
      r[0] = (int)l;
      return (l >> 32) & 1;
    }
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_uadd_n(const int* op1, const int* op2, int* r) {
      Ulong l = 0;
      for (int i = 0; i < N; i++) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_uadd_n<0>(const int* /*op1*/, const int* /*op2*/, int* /*r*/) { return false; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_uadd_n<1>(const int* op1, const int* op2, int* r) {
      Ulong l = (Ulong)(unsigned)op1[0] + (Ulong)(unsigned)op2[0];
      r[0] = (int)l;
      return (l >> 32) & 1;
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_uadd_n<2>(const int* op1, const int* op2, int* r) {
      Ulong l = (Ulong)(unsigned)op1[0] + (Ulong)(unsigned)op2[0];
      r[0] = (int)l;
      l >>= 32;
    ...
    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_add(const int* op1, const int* op2, int* r) {
      if (Nr == 1)
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_add<1, 1, 1>(const int* op1, const int* op2, int* r) {
      r[0] = (unsigned)op1[0] + (unsigned)op2[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_add<1, 1, 2>(const int* op1, const int* op2, int* r) {
      iv_assign_int64<2>(r, (Slong)op1[0] + (Slong)op2[0]);
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow(const int* op1, int op2, bool borrow, int* r) {
      if (N == 1) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow<0>(const int* /*op1*/, int /*op2*/, bool borrow, int* /*r*/) { return borrow; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow<1>(const int* op1, int op2, bool borrow, int* r) {
      Ulong l = (Slong)op1[0] - (Slong)op2 - borrow;
      r[0] = (int)l;
      return (l >> 32) & 1;
    }
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow(int op1, const int* op2, bool borrow, int* r) {
      if (N == 1) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow<0>(int /*op1*/, const int* /*op2*/, bool borrow, int* /*r*/) { return borrow; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_sub_int_borrow<1>(int op1, const int* op2, bool borrow, int* r) {
      Ulong l = (Slong)op1 - (Slong)op2[0] - borrow;
      r[0] = (int)l;
      return (l >> 32) & 1;
    }
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_usub_n(const int* op1, const int* op2, int* r) {
      Slong l = 0;
      for (int i = 0; i < N; i++) {
        l += (Ulong)(unsigned)op1[i] - (Ulong)(unsigned)op2[i];
        r[i] = (int)l;
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_usub_n<1>(const int* op1, const int* op2, int* r) {
      Ulong l = (Ulong)(unsigned)op1[0] - (Ulong)(unsigned)op2[0];
      r[0] = (int)l;
      return (l >> 32) & 1;
    }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_usub_n<2>(const int* op1, const int* op2, int* r) {
    ...
    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_sub(const int* op1, const int* op2, int* r) {
      if (Nr == 1)
        r[0] = (unsigned)op1[0] - (unsigned)op2[0];
      else {
        const int M1 = AC_MAX(N1, N2);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_sub<1, 1, 1>(const int* op1, const int* op2, int* r) {
      r[0] = (unsigned)op1[0] - (unsigned)op2[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_sub<1, 1, 2>(const int* op1, const int* op2, int* r) {
      iv_assign_int64<2>(r, (Slong)op1[0] - (Slong)op2[0]);
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline bool iv_all_bits_same(const int* op, bool bit) {
      int t = bit ? ~0 : 0;
      for (int i = 0; i < N; i++)
        if (op[i] != t)
          return false;
    ...
    template<> CUDA_CALLABLE_MEMBER inline bool iv_all_bits_same<0>(const int* /*op*/, bool /*bit*/) { return true; }
    template<> CUDA_CALLABLE_MEMBER inline bool iv_all_bits_same<1>(const int* op, bool bit) {
      return op[0] == (bit ? ~0 : 0);
    }

    template <int N, int Nr>
    CUDA_CALLABLE_MEMBER void iv_neg(const int* op1, int* r) {
      Slong l = 0;
    ...
    CUDA_CALLABLE_MEMBER void iv_udiv(const sw2* n, const sw2* d, sw2* q, sw2* r) {
    ...
    CUDA_CALLABLE_MEMBER inline Slong conv_to_Slong(const int* x) {
      return (Slong)(((Ulong)x[1] << 32) | (unsigned)x[0]);
    }

    template<int N1, int Num_s, int N2, int Den_s, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_div(const int* op1, const int* op2, int* r) {
    ...
    template<int N1, int Num_s, int N2, int Den_s, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_rem(const int* op1, const int* op2, int* r) {
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_complement_n(const int* op, int* r) {
      for (int i = 0; i < N; i++)
        r[i] = ~op[i];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_complement_n<1>(const int* op, int* r) {
      r[0] = ~op[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_complement_n<2>(const int* op, int* r) {
      r[0] = ~op[0];
      r[1] = ~op[1];
    }

    template<int N, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_complement(const int* op, int* r) {
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_and_n(const int* op1, const int* op2, int* r) {
      for (int i = 0; i < N; i++)
        r[i] = op1[i] & op2[i];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_and_n<1>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] & op2[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_and_n<2>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] & op2[0];
      r[1] = op1[1] & op2[1];
    }

    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_and(const int* op1, const int* op2, int* r) {
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_or_n(const int* op1, const int* op2, int* r) {
      for (int i = 0; i < N; i++)
        r[i] = op1[i] | op2[i];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_or_n<1>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] | op2[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_or_n<2>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] | op2[0];
      r[1] = op1[1] | op2[1];
    }

    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_or(const int* op1, const int* op2, int* r) {
      const int M1 = AC_MIN(AC_MAX(N1, N2), Nr);
      const int M2 = AC_MIN(AC_MIN(N1, N2), Nr);
      const int* OP1 = N1 >= N2 ? op1 : op2;
      const int* OP2 = N1 >= N2 ? op2 : op1;
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_xor_n(const int* op1, const int* op2, int* r) {
      for (int i = 0; i < N; i++)
        r[i] = op1[i] ^ op2[i];
    }
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_bitwise_xor_n<2>(const int* op1, const int* op2, int* r) {
      r[0] = op1[0] ^ op2[0];
      r[1] = op1[1] ^ op2[1];
    }

    template<int N1, int N2, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_bitwise_xor(const int* op1, const int* op2, int* r) {
      const int M1 = AC_MIN(AC_MAX(N1, N2), Nr);
      const int M2 = AC_MIN(AC_MIN(N1, N2), Nr);
      const int* OP1 = N1 >= N2 ? op1 : op2;
      const int* OP2 = N1 >= N2 ? op2 : op1;
    ...
    template<int N, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_shift_l(const int* op1, unsigned op2, int* r) {
      unsigned s31 = op2 & 31;
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_l<1, 1>(const int* op1, unsigned op2, int* r) {
      r[0] = op2 < 32 ? op1[0] << op2 : 0;
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_l<2, 1>(const int* op1, unsigned op2, int* r) {
      Ulong vop1 =
        (static_cast<Ulong>(op1[1]) << 32) | static_cast<unsigned int>(op1[0]);
      vop1 = op2 < 64 ? vop1 << op2 : (Ulong)0;
      r[0] = vop1;
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_l<2, 2>(const int* op1, unsigned op2, int* r) {
      Ulong vop1 =
        (static_cast<Ulong>(op1[1]) << 32) | static_cast<unsigned int>(op1[0]);
      vop1 = op2 < 64 ? vop1 << op2 : (Ulong)0;
    ...
    template<int N, int Nr>
    CUDA_CALLABLE_MEMBER inline void iv_shift_r(const int* op1, unsigned op2, int* r) {
      unsigned s31 = op2 & 31;
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_r<1, 1>(const int* op1, unsigned op2, int* r) {
      r[0] = (op2 < 32) ? (op1[0] >> op2) : (op1[0] >> 31);
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_r<2, 1>(const int* op1, unsigned op2, int* r) {
      Slong vop1 =
        (static_cast<Ulong>(op1[1]) << 32) | static_cast<unsigned int>(op1[0]);
      vop1 = (op2 < 64) ? (vop1 >> op2) : (vop1 >> 63);
      r[0] = vop1;
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_r<2, 2>(const int* op1, unsigned op2, int* r) {
      Slong vop1 =
        (static_cast<Ulong>(op1[1]) << 32) | static_cast<unsigned int>(op1[0]);
      vop1 = (op2 < 64) ? (vop1 >> op2) : (vop1 >> 63);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_l2<1, 1, false>(const int* op1, signed op2, int* r) {
      r[0] = (op2 < 32) ? ((unsigned)op1[0] << op2) : 0;
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_l2<1, 1, true>(const int* op1, signed op2, int* r) {
      r[0] = (op2 >= 0) ?
    ...
    template<int N, int Nr, bool S>
    CUDA_CALLABLE_MEMBER inline void iv_shift_r2(const int* op1, signed op2, int* r) {
      if (S && op2 < 0)
        iv_shift_l<N, Nr>(op1, -op2, r);
      else
        iv_shift_r<N, Nr>(op1, op2, r);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_r2<1, 1, false>(const int* op1, signed op2, int* r) {
      r[0] = (op2 < 32) ? (op1[0] >> op2) : (op1[0] >> 31);
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_shift_r2<1, 1, true>(const int* op1, signed op2, int* r) {
      r[0] = (op2 >= 0) ?
        (op2 < 32) ? (op1[0] >> op2) : (op1[0] >> 31) :
        (op2 > -32) ? ((unsigned)op1[0] << -op2) : 0;
    }
    ...
    CUDA_CALLABLE_MEMBER inline void iv_const_shift_l(const int* op1, int* r) {
      // B >= 0
      if (!B) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_const_shift_l<1, 1, 0>(const int* op1, int* r) {
      r[0] = op1[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_const_shift_l<2, 1, 0>(const int* op1, int* r) {
      r[0] = op1[0];
    }

    template<int N, int Nr, int B>
    CUDA_CALLABLE_MEMBER inline void iv_const_shift_r(const int* op1, int* r) {
      if (!B) {
        const int M1 = AC_MIN(N, Nr);
        iv_copy<M1>(op1, r);
        iv_extend<Nr - M1>(r + M1, r[M1 - 1] < 0 ? ~0 : 0);
    ...
    template<> CUDA_CALLABLE_MEMBER inline void iv_const_shift_r<1, 1, 0>(const int* op1, int* r) {
      r[0] = op1[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_const_shift_r<2, 1, 0>(const int* op1, int* r) {
      r[0] = op1[0];
    }

    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_conv_from_fraction(double d, int* r, bool* qb, bool* rbits, bool* o) {
      bool b = d < 0;
      double d2 = b ? -d : d;
    ...
    CUDA_CALLABLE_MEMBER inline int to_str(int* v, int w, bool left_just, char* r) {
    ...
    template<> CUDA_CALLABLE_MEMBER inline int to_str<AC_DEC>(int* v, int w, bool left_just, char* r) {
      int k = 0;
      int msw = (w - 1) >> 5;
      if (left_just) {
    ...
    CUDA_CALLABLE_MEMBER inline int to_string(int* v, int w, bool sign_mag, ac_base_mode base, bool left_just, bool pad_to_width, char* r) {
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline unsigned iv_leading_bits(const int* op, bool bit);

    template<> CUDA_CALLABLE_MEMBER inline unsigned iv_leading_bits<1>(const int* op, bool bit) {
    ...
    template<int W>
    CUDA_CALLABLE_MEMBER inline unsigned reverse_u(unsigned x) {
      unsigned r = x;
      if (W > 1) {
        int mask = 0x55555555;
        int shift = 1;
    ...
    template<int N>
    CUDA_CALLABLE_MEMBER inline void iv_reverse(const int* op, int* r) {
      for (int k = 0; k < N; k++)
        r[k] = reverse_u<32>((unsigned)op[N - 1 - k]);
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_reverse<1>(const int* op, int* r) {
      r[0] = reverse_u<32>((unsigned)op[0]);
    }
    template<> CUDA_CALLABLE_MEMBER inline void iv_reverse<2>(const int* op, int* r) {
      r[0] = reverse_u<32>((unsigned)op[1]);
      r[1] = reverse_u<32>((unsigned)op[0]);
    }

    CUDA_CALLABLE_MEMBER inline int set_bits_int(int op, int lsb, int WS, int slc) {
      // WS < 32, lsb+WS-1 < 32
    ...
    public:
      ...
      CUDA_CALLABLE_MEMBER iv(Slong t) {
        iv_assign_int64<N>(v, t);
      }
      CUDA_CALLABLE_MEMBER iv(Ulong t) {
        iv_assign_uint64<N>(v, t);
      }
      CUDA_CALLABLE_MEMBER iv(int t) {
        v[0] = t;
        iv_extend<N - 1>(v + 1, (t < 0) ? ~0 : 0);
      }
      CUDA_CALLABLE_MEMBER iv(unsigned int t) {
        v[0] = t;
        iv_extend<N - 1>(v + 1, 0);
      }
      CUDA_CALLABLE_MEMBER iv(long t) {
        if (long_w == 32) {
          v[0] = t;
          iv_extend<N - 1>(v + 1, (t < 0) ? ~0 : 0);
        }
      ...
      CUDA_CALLABLE_MEMBER iv(unsigned long t) {
        if (long_w == 32) {
          v[0] = t;
          iv_extend<N - 1>(v + 1, 0);
        }
      ...
      CUDA_CALLABLE_MEMBER iv(double d) {
      ...
      CUDA_CALLABLE_MEMBER inline Slong to_int64() const { return N == 1 ? v[0] : ((Ulong)v[1] << 32) | (Ulong)(unsigned)v[0]; }
      CUDA_CALLABLE_MEMBER inline Ulong to_uint64() const { return N == 1 ? (Ulong)v[0] : ((Ulong)v[1] << 32) | (Ulong)(unsigned)v[0]; }
      CUDA_CALLABLE_MEMBER inline double to_double() const {
        double a = v[N - 1];
        for (int i = N - 2; i >= 0; i--) {
          a *= (Ulong)1 << 32;
          a += (unsigned)v[i];
      ...
      CUDA_CALLABLE_MEMBER inline void conv_from_fraction(double d, bool* qb, bool* rbits, bool* o) {
        iv_conv_from_fraction<N>(d, v, qb, rbits, o);
      }

      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER inline void mult(const iv<N2>& op2, iv<Nr>& r) const {
        iv_mult<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER void add(const iv<N2>& op2, iv<Nr>& r) const {
        iv_add<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER void sub(const iv<N2>& op2, iv<Nr>& r) const {
        iv_sub<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int Num_s, int N2, int Den_s, int Nr>
      CUDA_CALLABLE_MEMBER void div(const iv<N2>& op2, iv<Nr>& r) const {
        iv_div<N, Num_s, N2, Den_s, Nr>(v, op2.v, r.v);
      }
      template<int Num_s, int N2, int Den_s, int Nr>
      CUDA_CALLABLE_MEMBER void rem(const iv<N2>& op2, iv<Nr>& r) const {
        iv_rem<N, Num_s, N2, Den_s, Nr>(v, op2.v, r.v);
      }
      CUDA_CALLABLE_MEMBER void increment() {
        iv_uadd_carry<N>(v, true, v);
      }
      CUDA_CALLABLE_MEMBER void decrement() {
        iv_sub_int_borrow<N>(v, 0, true, v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void neg(iv<Nr>& r) const {
        iv_neg<N, Nr>(v, r.v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void shift_l(unsigned op2, iv<Nr>& r) const {
        iv_shift_l<N, Nr>(v, op2, r.v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void shift_l2(signed op2, iv<Nr>& r) const {
        iv_shift_l2<N, Nr, true>(v, op2, r.v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void shift_r(unsigned op2, iv<Nr>& r) const {
        iv_shift_r<N, Nr>(v, op2, r.v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void shift_r2(signed op2, iv<Nr>& r) const {
        iv_shift_r2<N, Nr, true>(v, op2, r.v);
      }
      template<int Nr, int B>
      CUDA_CALLABLE_MEMBER void const_shift_l(iv<Nr>& r) const {
        iv_const_shift_l<N, Nr, B>(v, r.v);
      }
      template<int Nr, int B>
      CUDA_CALLABLE_MEMBER void const_shift_r(iv<Nr>& r) const {
        iv_const_shift_r<N, Nr, B>(v, r.v);
      }
      template<int Nr>
      CUDA_CALLABLE_MEMBER void bitwise_complement(iv<Nr>& r) const {
        iv_bitwise_complement<N, Nr>(v, r.v);
      }
      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER void bitwise_and(const iv<N2>& op2, iv<Nr>& r) const {
        iv_bitwise_and<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER void bitwise_or(const iv<N2>& op2, iv<Nr>& r) const {
        iv_bitwise_or<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int N2, int Nr>
      CUDA_CALLABLE_MEMBER void bitwise_xor(const iv<N2>& op2, iv<Nr>& r) const {
        iv_bitwise_xor<N, N2, Nr>(v, op2.v, r.v);
      }
      template<int N2>
      CUDA_CALLABLE_MEMBER bool equal(const iv<N2>& op2) const {
        return iv_equal<N, N2>(v, op2.v);
      }
      template<int N2>
      CUDA_CALLABLE_MEMBER bool greater_than(const iv<N2>& op2) const {
        return iv_compare<N, N2, true>(v, op2.v);
      }
      template<int N2>
      CUDA_CALLABLE_MEMBER bool less_than(const iv<N2>& op2) const {
        return iv_compare<N, N2, false>(v, op2.v);
      }
      CUDA_CALLABLE_MEMBER bool equal_zero() const {
        return iv_equal_zero<N>(v);
      }

      template<int N2>
      CUDA_CALLABLE_MEMBER void set_slc(unsigned lsb, int WS, const iv<N2>& op2) {
      ...
      CUDA_CALLABLE_MEMBER void reverse(iv<N>& r) const {
        iv_reverse<N>(v, r.v);
      }
      CUDA_CALLABLE_MEMBER unsigned leading_bits(bool bit) const {
        return iv_leading_bits<N>(v, bit);
      }
    };

    template<> CUDA_CALLABLE_MEMBER inline Slong iv<1>::to_int64() const { return v[0]; }
    template<> CUDA_CALLABLE_MEMBER inline Ulong iv<1>::to_uint64() const { return v[0]; }

    template<> CUDA_CALLABLE_MEMBER inline Slong iv<2>::to_int64() const {
      return ((Ulong)v[1] << 32) | (Ulong)(unsigned)v[0];
    }
    template<> CUDA_CALLABLE_MEMBER inline Ulong iv<2>::to_uint64() const {
      return ((Ulong)v[1] << 32) | (Ulong)(unsigned)v[0];
    }

    template<> template<> CUDA_CALLABLE_MEMBER inline void iv<1>::set_slc(unsigned lsb, int WS, const iv<1>& op2) {
      v[0] = WS == 32 ? op2.v[0] : set_bits_int(v[0], lsb, WS, op2.v[0]);
    }
    template<> template<> CUDA_CALLABLE_MEMBER inline void iv<2>::set_slc(unsigned lsb, int WS, const iv<1>& op2) {
      Ulong l = to_uint64();
      Ulong l2 = op2.to_uint64();
      l = set_bits_int64(l, lsb, WS, l2);  // WS <= 32, never full 64-bit assignment
      *this = l;
    }
    template<> template<> CUDA_CALLABLE_MEMBER inline void iv<2>::set_slc(unsigned lsb, int WS, const iv<2>& op2) {
      Ulong l = to_uint64();
      Ulong l2 = op2.to_uint64();
...
#pragma builtin
    ...
    CUDA_CALLABLE_MEMBER inline void bit_adjust() {
      const unsigned rem = (32 - W) & 31;
      Base::v[N - 1] = S ? ((signed)((unsigned)Base::v[N - 1] << rem) >> rem) : (rem ?
        ((unsigned)Base::v[N - 1] << rem) >> rem : 0);
    }

    CUDA_CALLABLE_MEMBER inline bool is_neg() const { return S && Base::v[N - 1] < 0; }

    // returns false if number is denormal
    template<int WE, bool SE>
    CUDA_CALLABLE_MEMBER bool normalize_private(ac_int<WE, SE>& exp, bool reserved_min_exp = false) {
      int expt = exp;
...
#pragma clang diagnostic ignored "-Wuninitialized"
    ...
    CUDA_CALLABLE_MEMBER inline ac_int& set_val() {
      const unsigned int all_ones = (unsigned)~0;
      if (V == AC_VAL_DC) {
        ac_int r;
        Base::operator =(r);
...
#pragma clang diagnostic pop
    ...
    // Explicit conversion functions to C built-in types -------------
    CUDA_CALLABLE_MEMBER inline int to_int() const { return Base::v[0]; }
    CUDA_CALLABLE_MEMBER inline unsigned to_uint() const { return Base::v[0]; }
    CUDA_CALLABLE_MEMBER inline long to_long() const {
      return ac_private::long_w == 32 ? (long)Base::v[0] : (long)Base::to_int64();
    }
    CUDA_CALLABLE_MEMBER inline unsigned long to_ulong() const {
      return ac_private::long_w == 32 ? (unsigned long)Base::v[0] : (unsigned long)Base::to_uint64();
    }
    CUDA_CALLABLE_MEMBER inline Slong to_int64() const { return Base::to_int64(); }
    CUDA_CALLABLE_MEMBER inline Ulong to_uint64() const { return Base::to_uint64(); }
    CUDA_CALLABLE_MEMBER inline double to_double() const { return Base::to_double(); }

    CUDA_CALLABLE_MEMBER inline int length() const { return W; }
    ...
    // Arithmetic : Binary ----------------------------------------------------
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::mult operator *(const ac_int<W2, S2>& op2) const {
      typename rt<W2, S2>::mult r;
      Base::mult(op2, r);
      return r;
    }
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::plus operator +(const ac_int<W2, S2>& op2) const {
      typename rt<W2, S2>::plus r;
      Base::add(op2, r);
      return r;
    }
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::minus operator -(const ac_int<W2, S2>& op2) const {
...
#pragma GCC diagnostic ignored "-Wsign-compare"
#endif
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::div operator /(const ac_int<W2, S2>& op2) const {
      typename rt<W2, S2>::div r;
      enum {
        Nminus = ac_int<W + S, S>::N, N2 = ac_int<W2, S2>::N, N2minus = ac_int<W2 + S2, S2>::N,
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::mod operator %(const ac_int<W2, S2>& op2) const {
...
#pragma GCC diagnostic pop
    ...
    // Arithmetic assign  ------------------------------------------------------
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER ac_int& operator *=(const ac_int<W2, S2>& op2) {
      Base r;
      Base::mult(op2, r);
      Base::operator=(r);
      bit_adjust();
...
#pragma GCC diagnostic pop
    ...
    CUDA_CALLABLE_MEMBER ac_int& operator ++() {
      Base::increment();
      bit_adjust();
      return *this;
    }
    ...
    CUDA_CALLABLE_MEMBER const ac_int operator ++(int) {
      ac_int t = *this;
      Base::increment();
      bit_adjust();
      return t;
    ...
    // Arithmetic Unary --------------------------------------------------------
    CUDA_CALLABLE_MEMBER ac_int operator +() const {
      return *this;
    }
    CUDA_CALLABLE_MEMBER typename rt_unary::neg operator -() const {
    ...
    CUDA_CALLABLE_MEMBER ac_int<W + !S, true> operator ~() const {
      ac_int<W + !S, true> r;
      Base::bitwise_complement(r);
      return r;
    }
    // Bitwise (non-arithmetic) bit_complement  -----------------------------
    CUDA_CALLABLE_MEMBER ac_int<W, false> bit_complement() const {
      ac_int<W, false> r;
      Base::bitwise_complement(r);
      r.bit_adjust();
      return r;
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER typename rt<W2, S2>::logic operator & (const ac_int<W2, S2>& op2) const {
    ...
    template<int W2>
    CUDA_CALLABLE_MEMBER ac_int operator << (const ac_int<W2, true>& op2) const {
      ac_int r;
    ...
    template<int W2>
    CUDA_CALLABLE_MEMBER ac_int operator << (const ac_int<W2, false>& op2) const {
      ac_int r;
    ...
    // Shift assign ------------------------------------------------------------
    template<int W2>
    CUDA_CALLABLE_MEMBER ac_int& operator <<= (const ac_int<W2, true>& op2) {
      Base r;
      Base::shift_l2(op2.to_int(), r);
      Base::operator=(r);
      bit_adjust();
    ...
    template<int W2>
    CUDA_CALLABLE_MEMBER ac_int& operator <<= (const ac_int<W2, false>& op2) {
      Base r;
      Base::shift_l(op2.to_uint(), r);
      Base::operator=(r);
      bit_adjust();
    ...
    // Relational ---------------------------------------------------------------
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER bool operator == (const ac_int<W2, S2>& op2) const {
    ...
    CUDA_CALLABLE_MEMBER inline const ac_int<WS, S> slc(const ac_int<WX, SX>& index) const {
      ac_int<WS, S> r;
    ...
    template<int WS>
    CUDA_CALLABLE_MEMBER inline const ac_int<WS, S> slc(signed index) const {
      ac_int<WS, S> r;
    ...
    template<int WS>
    CUDA_CALLABLE_MEMBER inline const ac_int<WS, S> slc(unsigned uindex) const {
      ac_int<WS, S> r;
      Base::shift_r(uindex, r);
      r.bit_adjust();
      return r;
    ...
    CUDA_CALLABLE_MEMBER inline ac_int& set_slc(const ac_int<WX, SX> lsb, const ac_int<W2, S2>& slc) {
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER inline ac_int& set_slc(signed lsb, const ac_int<W2, S2>& slc) {
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER inline ac_int& set_slc(unsigned ulsb, const ac_int<W2, S2>& slc) {
    ...
    CUDA_CALLABLE_MEMBER inline ac::sliceref<Msb - Lsb + 1, Lsb, Msb == W - 1, S> range() {
...
# endif
      ...
      unsigned d_index;
    public:
      CUDA_CALLABLE_MEMBER ac_bitref(ac_int* bv, unsigned index = 0) : d_bv(*bv), d_index(index) {}
      CUDA_CALLABLE_MEMBER operator bool() const { return (d_index < W) ? (d_bv.v[d_index >> 5] >> (d_index & 31) & 1) : 0; }

      template<int W2, bool S2>
      CUDA_CALLABLE_MEMBER operator ac_int<W2, S2>() const { return operator bool(); }

      CUDA_CALLABLE_MEMBER inline ac_bitref operator = (int val) {
      ...
      template<int W2, bool S2>
      CUDA_CALLABLE_MEMBER inline ac_bitref operator = (const ac_int<W2, S2>& val) {
        return operator =(val.to_int());
      }
      CUDA_CALLABLE_MEMBER inline ac_bitref operator = (const ac_bitref& val) {
        return operator =((int)(bool)val);
      }
    };

    CUDA_CALLABLE_MEMBER ac_bitref operator [] (unsigned int uindex) {
      AC_ASSERT(uindex < W, "Attempting to read bit beyond MSB");
      ac_bitref bvh(this, uindex);
      return bvh;
    }
    CUDA_CALLABLE_MEMBER ac_bitref operator [] (int index) {
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER ac_bitref operator [] (const ac_int<W2, S2>& index) {
    ...
    CUDA_CALLABLE_MEMBER bool operator [] (unsigned int uindex) const {
      AC_ASSERT(uindex < W, "Attempting to read bit beyond MSB");
      return (uindex < W) ? (Base::v[uindex >> 5] >> (uindex & 31) & 1) : 0;
    }
    CUDA_CALLABLE_MEMBER bool operator [] (int index) const {
      AC_ASSERT(index >= 0, "Attempting to read bit with negative index");
      unsigned uindex = index & ((unsigned)~0 >> 1);
      AC_ASSERT(uindex < W, "Attempting to read bit beyond MSB");
      return (uindex < W) ? (Base::v[uindex >> 5] >> (uindex & 31) & 1) : 0;
    ...
    template<int W2, bool S2>
    CUDA_CALLABLE_MEMBER bool operator [] (const ac_int<W2, S2>& index) const {
      AC_ASSERT(index.to_int() >= 0, "Attempting to read bit with negative index");
      unsigned uindex = ac_int<W2 - S2, false>(index).to_uint();
      AC_ASSERT(uindex < W, "Attempting to read bit beyond MSB");
      return (uindex < W) ? (Base::v[uindex >> 5] >> (uindex & 31) & 1) : 0;
    ...
    CUDA_CALLABLE_MEMBER void reverse() {
      if (W > 32) {
    ...
    CUDA_CALLABLE_MEMBER ac_int<W, false> reversed() const {
      if (W > 32) {
        typedef ac_int<W, true> intW_t;
        typename intW_t::Base r0(*this);
        intW_t r;
    ...
    CUDA_CALLABLE_MEMBER typename rt_unary::leading_sign leading_sign() const {
      unsigned ls = Base::leading_bits(S & (Base::v[N - 1] < 0)) - (32 * N - W) - S;
      return ls;
    }
    CUDA_CALLABLE_MEMBER typename rt_unary::leading_sign leading_sign(bool& all_sign) const {
      unsigned ls = Base::leading_bits(S & (Base::v[N - 1] < 0)) - (32 * N - W) - S;
      all_sign = (ls == W - S);
      return ls;
    }
    // returns false if number is denormal
    template<int WE, bool SE>
    CUDA_CALLABLE_MEMBER bool normalize(ac_int<WE, SE>& exp) {
    ...
    template<int WE, bool SE>
    CUDA_CALLABLE_MEMBER bool normalize_RME(ac_int<WE, SE>& exp) {
      return normalize_private(exp, true);
    }
    CUDA_CALLABLE_MEMBER bool and_reduce() const {
      return ac_private::iv_equal_ones_to<W, N>(Base::v);
    }
    CUDA_CALLABLE_MEMBER bool or_reduce() const {
      return !Base::equal_zero();
    }
    CUDA_CALLABLE_MEMBER bool xor_reduce() const {
    ...
    CUDA_CALLABLE_MEMBER inline void bit_fill_hex(const char* str) {
    ...
    template<int Na>
    CUDA_CALLABLE_MEMBER inline void bit_fill(const int(&ivec)[Na], bool bigendian = true) {
  ...
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, true>::ac_int(bool b) { v[0] = b ? -1 : 0; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(bool b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(signed char b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(unsigned char b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(signed short b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(unsigned short b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(signed int b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(unsigned int b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(signed long b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(unsigned long b) { v[0] = b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(Ulong b) { v[0] = (int)b & 1; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<1, false>::ac_int(Slong b) { v[0] = (int)b & 1; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, true>::ac_int(bool b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, false>::ac_int(bool b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, true>::ac_int(signed char b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, false>::ac_int(unsigned char b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, true>::ac_int(unsigned char b) { v[0] = (signed char)b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<8, false>::ac_int(signed char b) { v[0] = (unsigned char)b; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, true>::ac_int(bool b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, false>::ac_int(bool b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, true>::ac_int(signed char b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, false>::ac_int(unsigned char b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, true>::ac_int(unsigned char b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, false>::ac_int(signed char b) { v[0] = (unsigned short)b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, true>::ac_int(signed short b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, false>::ac_int(unsigned short b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, true>::ac_int(unsigned short b) { v[0] = (signed short)b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<16, false>::ac_int(signed short b) { v[0] = (unsigned short)b; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, true>::ac_int(signed int b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, true>::ac_int(unsigned int b) { v[0] = b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, false>::ac_int(signed int b) { v[0] = b; v[1] = 0; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, false>::ac_int(unsigned int b) { v[0] = b; v[1] = 0; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, true>::ac_int(Slong b) { v[0] = (int)b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, true>::ac_int(Ulong b) { v[0] = (int)b; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, false>::ac_int(Slong b) { v[0] = (int)b; v[1] = 0; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<32, false>::ac_int(Ulong b) { v[0] = (int)b; v[1] = 0; }

  template<> CUDA_CALLABLE_MEMBER inline ac_int<64, true>::ac_int(Slong b) { v[0] = (int)b; v[1] = (int)(b >> 32); }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<64, true>::ac_int(Ulong b) { v[0] = (int)b; v[1] = (int)(b >> 32); }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<64, false>::ac_int(Slong b) { v[0] = (int)b; v[1] = (int)((Ulong)b >> 32); v[2] = 0; }
  template<> CUDA_CALLABLE_MEMBER inline ac_int<64, false>::ac_int(Ulong b) { v[0] = (int)b; v[1] = (int)(b >> 32); v[2] = 0; }
...
#define BIN_OP_WITH_INT(BIN_OP, C_TYPE, WI, SI, RTYPE)  \
  template<int W, bool S> \
  CUDA_CALLABLE_MEMBER inline typename ac_int<WI,SI>::template rt<W,S>::RTYPE operator BIN_OP ( C_TYPE i_op, const ac_int<W,S> &op) {  \
    return ac_int<WI,SI>(i_op).operator BIN_OP (op);  \
  } \
  template<int W, bool S>   \
  CUDA_CALLABLE_MEMBER inline typename ac_int<W,S>::template rt<WI,SI>::RTYPE operator BIN_OP ( const ac_int<W,S> &op, C_TYPE i_op) {  \
    return op.operator BIN_OP (ac_int<WI,SI>(i_op));  \
  }

#define REL_OP_WITH_INT(REL_OP, C_TYPE, W2, S2)  \
  template<int W, bool S>   \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( const ac_int<W,S> &op, C_TYPE op2) {  \
    return op.operator REL_OP (ac_int<W2,S2>(op2));  \
  }  \
  template<int W, bool S> \
  CUDA_CALLABLE_MEMBER inline bool operator REL_OP ( C_TYPE op2, const ac_int<W,S> &op) {  \
    return ac_int<W2,S2>(op2).operator REL_OP (op);  \
  }

#define ASSIGN_OP_WITH_INT(ASSIGN_OP, C_TYPE, W2, S2)  \
  template<int W, bool S>   \
  CUDA_CALLABLE_MEMBER inline ac_int<W,S> &operator ASSIGN_OP ( ac_int<W,S> &op, C_TYPE op2) {  \
...
// for backward compatability with v3.9.0 and earlier define following macro
  ...
  namespace ac {
    ...
    template<typename T>
    CUDA_CALLABLE_MEMBER inline T bit_fill_hex(const char* str) {
      T res;
    ...
    CUDA_CALLABLE_MEMBER inline T bit_fill(const int(&ivec)[N], bool bigendian = true) {
      T res;
  ...
  CUDA_CALLABLE_MEMBER T* operator +(T* ptr, const ac_int<W, S>& op2) {
    return ptr + op2.to_int64();
  }
  template<typename T, int W, bool S>
  CUDA_CALLABLE_MEMBER T* operator +(const ac_int<W, S>& op2, T* ptr) {
    return ptr + op2.to_int64();
  }
...
// Global templatized functions for easy initialization to special values
  ...
  CUDA_CALLABLE_MEMBER inline ac_int<W, S> value(ac_int<W, S>) {
    ac_int<W, S> r;
  ...
  CUDA_CALLABLE_MEMBER inline ac_fixed<W, I, S, Q, O> value(ac_fixed<W, I, S, Q, O>);
...
template<ac_special_val val> CUDA_CALLABLE_MEMBER inline C_TYPE value(C_TYPE); \
template<> CUDA_CALLABLE_MEMBER inline C_TYPE value<AC_VAL_0>(C_TYPE) { return (C_TYPE)0; } \
SPECIAL_VAL_FOR_INTS_DC(C_TYPE, WI, SI) \
template<> CUDA_CALLABLE_MEMBER inline C_TYPE value<AC_VAL_QUANTUM>(C_TYPE) { return (C_TYPE)1; } \
template<> CUDA_CALLABLE_MEMBER inline C_TYPE value<AC_VAL_MAX>(C_TYPE) { return (C_TYPE)(SI ? ~(((C_TYPE) 1) << (WI-1)) : (C_TYPE) -1); } \
template<> CUDA_CALLABLE_MEMBER inline C_TYPE value<AC_VAL_MIN>(C_TYPE) { return (C_TYPE)(SI ? ((C_TYPE) 1) << (WI-1) : (C_TYPE) 0); }
